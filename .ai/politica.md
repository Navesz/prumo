<!-- FONTE CANÔNICA das instruções de IA do Prumo.
     Editar AQUI. CLAUDE.md, AGENTS.md e .cursor/rules/prumo.mdc são GERADOS:
       node .ai/gerar.mjs
     `node .ai/gerar.mjs --verificar` não escreve nada e reprova se alguma cópia
     divergir desta fonte. É o primeiro passo do `verificar`, antes do typecheck,
     porque instrução desatualizada estraga tudo que roda depois dela.
     As seções são recortadas pelos marcadores `inicio:` / `fim:`. Texto fora de
     seção reprova o gerador de propósito — regra que não entra em saída nenhuma
     é regra que não existe, e existir só na fonte é o modo silencioso de falhar. -->
